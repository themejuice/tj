...
Yes! By default, `tj` sets up all project's to support [xip.io](http://xip.io/) (definitely check it out). To test a project from another device on the same network, go to `<project-name>.<your-hosts-ip-address>.xip.io` e.g. `themejuice.192.168.1.1.xip.io`.
