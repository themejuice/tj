...
module ThemeJuice
  class Service
    ...
    def restart_vagrant
      ...
      run ["cd #{@environment.vvv_path}", "vagrant halt"]

      # Up, up, up!
      if run ["cd #{@environment.vvv_path}", "vagrant up --provision"], false
        true
      else
        false
      end
    ...
    def database_is_setup?
      File.readlines(File.expand_path("#{@environment.vvv_path}/database/init-custom.sql")).grep(/(#(#*)? Begin '#{@opts[:project_name]}')/m).any?
    end
    ...
    def synced_folder_is_setup?
      File.readlines(File.expand_path("#{@environment.vvv_path}/Vagrantfile")).grep(/(#(#*)? Begin '#{@opts[:project_name]}')/m).any?
    end
