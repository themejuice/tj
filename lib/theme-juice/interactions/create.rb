    ...
    class << self
      ...
      def setup_project_name
        if @environment.yolo
          ...
          name = @interaction.prompt "What's the project name? (letters, numbers and dashes only)"
        end
      ...
      def setup_project_location
        ...
        if @opts[:use_defaults]
          ...
          location = File.expand_path(@interaction.prompt "Where do you want to setup the project?", :default => path, :path => true)
        end
      ...
      def setup_project_starter_theme
        if @opts[:project_bare]
          ...
          when "none"
            @interaction.notice "Next time you need to create a project without a starter theme, you can just run the 'setup' command instead."
      ...
      def setup_project_dev_url
        if @opts[:use_defaults]
          ...
          url = @interaction.prompt "What do you want the development url to be? (this should end in '.dev')", :default => "#{@opts[:project_name]}.dev"
        end
      ...
      def setup_project_db_name
        if @opts[:use_defaults] || @opts[:skip_db]
          ...
          db_name = @interaction.prompt "Database name", :default => "#{@opts[:project_name_clean]}_db"
        end
      ...
      def setup_project_db_user
        if @opts[:use_defaults] || @opts[:skip_db]
          ...
          db_user = @interaction.prompt "Database username", :default => "#{@opts[:project_name_clean]}_user"
        end
      ...
      def validate_project_name(name)
        if name.empty?
          @interaction.error "Project name '#{name}' is invalid or empty. Aborting mission."
        end

        "#{name}".match /[^0-9A-Za-z.\-]/ do |char|
          @interaction.error "Project name contains an invalid character '#{char}'. This name is used for creating directories, so that's not gonna work. Aborting mission."
        end
      end
